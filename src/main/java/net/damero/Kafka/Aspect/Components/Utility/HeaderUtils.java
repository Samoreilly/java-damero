 ...
 * This allows users to receive their original event object with metadata stored
 * in headers.
 */
public class HeaderUtils {
    ...
    /**
     * Ensures the Spring Kafka type header (__TypeId__) is present on the provided
     * headers.
     * Priority: explicitType (from annotation) -> originalEvent.getClass() ->
     * no-op.
    ...
    /**
     * Writes metadata from an EventMetadata object directly into Kafka headers.
     * Use this for "exploding" an internal EventWrapper into its constituent parts.
     */
    public static void writeMetadataToHeaders(Headers headers, EventMetadata metadata) {
        if (headers == null || metadata == null)
            return;

        // Skip if already has damero headers to avoid duplication
        if (hasRetryHeaders(headers))
            return;

        headers.add(HEADER_ATTEMPTS, String.valueOf(metadata.getAttempts()).getBytes(StandardCharsets.UTF_8));

        if (metadata.getFirstFailureDateTime() != null) {
            headers.add(HEADER_FIRST_FAILURE_DATETIME,
                    metadata.getFirstFailureDateTime().format(DATETIME_FORMATTER).getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getLastFailureDateTime() != null) {
            headers.add(HEADER_LAST_FAILURE_DATETIME,
                    metadata.getLastFailureDateTime().format(DATETIME_FORMATTER).getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getOriginalTopic() != null) {
            headers.add(HEADER_ORIGINAL_TOPIC, metadata.getOriginalTopic().getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getDlqTopic() != null) {
            headers.add(HEADER_DLQ_TOPIC, metadata.getDlqTopic().getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getDelayMs() != null) {
            headers.add(HEADER_DELAY_MS, String.valueOf(metadata.getDelayMs()).getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getDelayMethod() != null) {
            headers.add(HEADER_DELAY_METHOD, metadata.getDelayMethod().name().getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getMaxAttempts() != null) {
            headers.add(HEADER_MAX_ATTEMPTS,
                    String.valueOf(metadata.getMaxAttempts()).getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getFirstFailureException() != null) {
            headers.add(HEADER_FIRST_FAILURE_EXCEPTION,
                    serializeException(metadata.getFirstFailureException()).getBytes(StandardCharsets.UTF_8));
        }

        if (metadata.getLastFailureException() != null) {
            headers.add(HEADER_LAST_FAILURE_EXCEPTION,
                    serializeException(metadata.getLastFailureException()).getBytes(StandardCharsets.UTF_8));
        }
    }
     ...
     * 
     * @param existingMetadata the existing metadata extracted from headers (may be
     *                         null)
    ...
    public static RecordHeaders buildHeadersFromMetadata(EventMetadata existingMetadata,
        ...
        LocalDateTime firstFailureDateTime = existingMetadata != null
                && existingMetadata.getFirstFailureDateTime() != null
        ...
        Exception firstFailureException = existingMetadata != null
                && existingMetadata.getFirstFailureException() != null
